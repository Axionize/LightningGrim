    ...
    private static void checkForUpdates(Sender sender) {
        ...
        try {
            ...
            Component msg = switch (status) {
                case AHEAD ->
                        Component.text("You are using a development version of GrimAC (Custom Fork by Bram)").color(NamedTextColor.LIGHT_PURPLE);
                case UPDATED ->
                        Component.text("You are using the latest version of GrimAC (Custom Fork by Bram)").color(NamedTextColor.GREEN);
